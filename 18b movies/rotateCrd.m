...
% for Claire's Kif18B RNAi movies.  This script is the first step of
% rotating the spindles and getting the coordinates saved in
% <RotTracks.mat>. It goes through folders in the direction to process each
% spindle information. 

% Note: Go to directory  /Users/hailingzong/Documents/MATLAB/18b
% 
% Process: loop 
% 1) Load projData.mat from roi1/meta
% 2) Input the positions of two poles. Calculate the  rotation angle theta.
% 3) Rotate around the center point of the image to make the spindle horizontal
% 4) Use Matrix rotation to rotate projData.xCoord and projData.yCoordand with 
%    'theta' around 'center':image (256,256)
% 5) Input spindle boundary as box_perm: top, bottom, left, right, midline
% 6) Save all tracks info in <RotTracks.mat>
...
%% Save coordinates of pole positions in field contains both x and y to save space. Added 2016/3/31
%  Remember to indicate (1) or (2) as x and y. 
for n = 1:length( RotTracks )
    RotTracks(n).Pole_left = [ RotTracks(n).Pole_left_x, RotTracks(n).Pole_left_y ]; 
    RotTracks(n).Pole_right = [ RotTracks(n).Pole_right_x, RotTracks(n).Pole_right_y ]; 
    RotTracks(n).Pole_left_r = [ RotTracks(n).Pole_left_xr, RotTracks(n).Pole_left_yr ]; 
    RotTracks(n).Pole_right_r = [ RotTracks(n).Pole_right_xr, RotTracks(n).Pole_right_yr ]; 
end 
fields = {'Pole_left_x','Pole_left_y', 'Pole_right_x', 'Pole_right_y', 'Pole_left_xr', 'Pole_left_yr', 'Pole_right_xr', 'Pole_right_yr'};
RotTracks = rmfield(RotTracks,fields);
